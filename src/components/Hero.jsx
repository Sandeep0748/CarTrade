...
const Hero = () => {
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    navigate(
      `/cars?city=${selectedCity}&carType=${carType}&budget=${budget}`
    );
  };
  ...
  return (
    ...
    >
      ...
      >
        Buy & Sell Cars with{" "}
        <span className="text-blue-600">CarTrade</span>
      </motion.h1>
      ...
      <motion.form
        ...
        className="flex flex-col md:flex-row items-center justify-between gap-6 p-6 rounded-2xl md:rounded-full w-full max-w-5xl bg-white/80 backdrop-blur-md shadow-xl border border-gray-200"
      >
        {/* City Selection */}
        <div className="relative w-full md:w-1/3">
          <img
            ...
            alt="location"
            className="absolute left-3 top-3 h-5 text-gray-400"
          />
          <select
            required
            value={selectedCity}
        ...
        <div className="relative w-full md:w-1/3">
          <img
            src={assets.currency_icon || assets.fuel_icon}
            alt="budget"
            className="absolute left-3 top-3 h-5 text-gray-400"
          />
          <input
      ...
      <motion.img
        ...
        src={assets.main_car}
        alt="car"
        className="max-h-74 drop-shadow-xl"
      />
    </motion.div>
  );
};
